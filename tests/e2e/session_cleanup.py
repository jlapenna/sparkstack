...
def cleanup_session(session_id: str) -> None:
    ...
    if not SESSIONS_INDEX.exists():
        logger.debug(f"Sessions index not found at {SESSIONS_INDEX}, skipping cleanup.")
        return
    ...
    if not isinstance(sessions, list):
        logger.debug("Sessions index has unexpected format, skipping cleanup.")
        return
    ...
    if not uuids_to_remove:
        logger.debug(f"No sessions found matching '{session_id}', skipping cleanup.")
        return
    ...
    for sid in uuids_to_remove:
        for suffix in [".jsonl", ".trajectory.jsonl", ".trajectory-path.json"]:
            ...
            if transcript.exists():
                transcript.unlink()
                logger.debug(f"Removed transcript: {transcript.name}")
...
def wipe_all_sessions(label: str = "wipe") -> None:
    ...
    if not VERIFIER_SESSIONS_DIR.exists():
        logger.debug(f"[{label}] No verifier sessions directory found, skipping.")
        return
    count = 0
    for f in VERIFIER_SESSIONS_DIR.iterdir():
        if f.is_file():
            f.unlink()
            count += 1
    logger.info(f"[{label}] Removed {count} verifier session file(s) from {VERIFIER_SESSIONS_DIR}")

...
The monitoring stack is defined in `monitoring/docker-compose.yml` and consists of:

- **Prometheus** (9090): Time-series database. Configuration is dynamically generated by `uv run python -m scripts.build_stack` and symlinked to `current/prometheus.yml`. Scrape interval is 15s.
- **Grafana** (3001): Visualization. Dashboards are auto-provisioned from `monitoring/grafana/provisioning/dashboards/`.
...
- **Run Updates:** Use the `uv run python -m scripts.update_monitoring` script. It queries the GitHub, Docker Hub, and GCR APIs to find the true latest stable semantic versions. It also switches Docker Hub images to `quay.io` where possible to avoid API rate limits, and pins them in `docker-compose.yml`.
...
Scan the provisioning directory (e.g. `monitoring/grafana/provisioning/dashboards/`) and categorize the JSON files into:

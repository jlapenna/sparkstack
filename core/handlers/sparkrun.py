...
from core.registry import resolve_latest_image_tag
from core.schemas import SparkrunRegistryModel
...
class SparkrunServiceHandler:
    ...
    def apply_to_builders(
        ...
        max_len = int(
            vllm_cfg.get(
                "max_model_len", self.overrides.get("max_model_len", DEFAULT_CONTEXT_WINDOW)
            )
        )
        ...
        logger.info(
            f"🪄  Orchestrating {self.recipe_name} via sparkrun (Port: {self.port}, Mem: {mem_limit})"
        )

        # Memory GB requires parsing, we assume Sparkrun consumes very little RAM relative to VRAM, but handle it:
        mem_gb = 0.0  # Will be handled by check constraints properly from total VRAM
